---
title: The March Timeline
description: Multiple threads point back to the same month.
entryType: mystery
status: Largely confirmed
tags: [investigation]
case: Operation ALICE
evidenceDate: "20-31 Mar 1995"
sessions: [2, 3, 6, 7]
---

## What Happened

What began as a coincidence in Session 2 turned out to be a real, sharp turning point for the entire building:

- [Abigail Wright's mail](/evidence/operation-alice/abigails-mail/) shows utility bills lapsing from March, yet power and water still run.
- Food in [Thomas Manuel's fridge](/npcs/suspects-witnesses/thomas-manuel/) is fresh despite expiry dates from late March.
- Per [Manuel's mail](/evidence/operation-alice/manuels-mail/), his parents were turned away from the building on **31 March**, after dark, and told he no longer lived there.
- Per FBI records checked in Session 6, tenants across the building stopped making calls or spending money at all from around March onward.
- Cable repairman [David Langford](/npcs/suspects-witnesses/david-langford/) confirmed he entered the building on **20 March 1995** and became trapped in the Night Floors from that date.

## Why It's Flagged

Multiple independent tenants and records all converge on the same roughly two-week window in March 1995 — well before Wright's August disappearance and the FBI's involvement. Something happened to the building itself during this window.

## Current Theories

- The Night Floors likely became active, or significantly more dangerous, sometime in mid-to-late March 1995, trapping or absorbing residents from that point on. Wright's own disappearance in August may simply be the point at which someone finally noticed and filed a report — not when anything actually began.

## Status

**Largely confirmed.** The date range is well-supported by multiple independent sources; the underlying cause (what specifically happened in March) is still unknown.
